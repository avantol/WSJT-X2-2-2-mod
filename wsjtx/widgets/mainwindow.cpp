...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  //m_externalCtrl {false}      //avt 12/5/20
  ...
  connect (m_messageClient, &MessageClient::setup_tx, [this] (int newTxMsgIdx, QString const& msg, bool skipGrid, bool useRR73, QString const& check) {    //avt 11/16/20
      ...
      if (newTxMsgIdx) {          //perform some QSO state action (mostly)
        ...
        if (newTxMsgIdx == 6) {   //set up some kind of CQ, set options
          ...
          ui->genMsg->setText(ui->tx6->text());   //update generated msg
...
//  ::memcpy(dec_data.params.hiscall,(Radio::base_callsign (hisCall) +  "            ").toLatin1 ().constData (), sizeof dec_data.params.hiscall);
  ...
  if(bmsk144 and (line[0]!=0)) {
    ...
    if (m_mode != "ISCAT") postDecode (true, decodedtext);    //avt 12/5/20
//    writeAllTxt(message);
...
//Left (Band activity) window
    ...
    postDecode (true, decodedtext);    //avt 12/5/20
    write_all("Rx",message);
...
void MainWindow::auto_sequence (DecodedText const& message, unsigned start_tolerance, unsigned stop_tolerance)
{
  ...
  if (message_words.size () > 2 && (message.isStandardMessage() || (is_73 or is_OK))) {
    ...
    if (m_auto
      ...
      //ui->stopTxButton->click (); // halt any transmission  avt 11/17/20  not necessary, interferes with UDP listener actions
...
//                                    22, 22);
    ...
    if (m_sentFirst73) {
      ...
      if((m_config.prompt_to_log() or m_config.autoLog()) && !m_tune && !m_externalCtrl) logQSOTimer.start(0);    //avt 22/5/20 all logging done by external ctrl
    }
    ...
    if(is_73 and (m_config.disable_TX_on_73() or b)) {
      ...
      if(m_nextCall!="") {
        ...
        if (!m_externalCtrl) auto_tx_mode (false);    //avt 11/20/20 leave Tx enabled fo UDP listener next action
...
{
  m_txFirst = (nstate==2);
  statusUpdate();           //avt 11/19/20 UDP listener needs this info
}
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if(message.string().contains(hiscall+"/R")) {
    ...
    statusUpdate();     //avt 11/21/20 so that UDP listener is notified
  }
...
{
  if (SpecOp::FOX != m_config.special_op_id () && !m_externalCtrl)     //avt 11/21/20 leave Tx enabled for UDP listener next action
    {
  ...
  m_logDlg->initLogQSO (m_hisCall, grid, m_modeTx, m_rptSent, m_rptRcvd,
                        ...
                        ui->TxFreqSpinBox->value(), m_noSuffix, m_xSent, m_xRcvd, m_externalCtrl);    //avt 11/20/20
  m_inQSOwith="";
}
                            ...
                            , QByteArray const& ADIF)
{
  ...
  last_tx_label.setText("Logged QSO: " + m_hisCall);    //avt 11/20/20 UDP listener needs UI feedback
                            ...
                            , bool useReply, quint8 modifiers)      //avt new 12/27/20
{
  ...
  QTextCursor cursor;    //avt 12/21/20
  if (!m_externalCtrl) {        //avt 12/21/20 use message as rec'd here
  ...
  if (!cursor.isNull () || m_externalCtrl) //avt 11/28/20
    {
      ...
      //if (message_text.contains (QRegularExpression {R"(^(CQ |CQDX |QRZ ))"})) {}      //avt 11/15/20 
      //}
      ...
      if (message.size() >= 4 && message.left (4) != "----")
        {
          ...
          if (parts.size () >= 5 && parts[3].contains ('.')) // WSPR
          ...
          else
            {
              ...
              postDecode (false, DecodedText {message});      //avt 12/5/20
            }
        }
    }
  statusChanged ();
...
void MainWindow::initExternalCtrl()    //avt 12/5/20
{
  if (is_externalCtrlMode())
  {
    ...
    ui->cbFirst->setChecked(false);   //avt 11/22/20
    ui->cbFirst->setEnabled(false);   //avt 11/22/20
  } else {
    ui->cbAutoSeq->setEnabled(true); //avt 11/22/20
    ui->cbFirst->setEnabled(true);   //avt 11/22/20
  }
