            ...
            case NetworkMessage::Status:
              {
                ...
                QByteArray lastTxMsg;       //avt 11/19/20
                quint32 qsoProgress {0};    //avt 11/19/20
                bool txFirst {false};       //avt 11/19/20
                ...
                if (check_status (in) != Fail)
                  {
                    Q_EMIT self_->status_update (id, f, QString::fromUtf8 (mode), QString::fromUtf8 (dx_call)
                                                 ...
                                                 , QString::fromUtf8 (lastTxMsg)    //avt 11/19/20
                                                 , qsoProgress                      //avt 11/19/20
                                                 , txFirst);                         //avt 11/19/20
...
void MessageServer::setup_tx (QString const& id, quint32 nextMsgIdx)       //avt 11/16/20
{
  auto iter = m_->clients_.find (id);
  if (iter != std::end (m_->clients_))
    {
      QByteArray message;
      NetworkMessage::Builder out {&message, NetworkMessage::SetupTx, id, (*iter).negotiated_schema_number_};
      out << nextMsgIdx;
      m_->send_message (out, message, iter.value ().sender_address_, (*iter).sender_port_);
    }
}
